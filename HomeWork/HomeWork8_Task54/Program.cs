...
void RowSort (int[,] array, int n) // Сортирует элементы строки n в array по возрастанию
{
    ...
    {   
    int min = i;                            // Задаем индекс минимального элемента строки
    for (int j=i;j<array.GetLength(1);j++)  // Ищем минимальный элемент в неотсорт. части
    {
        
        if (array[n,j]<array[n,min])
        {
            min = j;                        // При нахождении нового мин. элемента
        }                                   // заменяем индекс
    }

    if (array[n,min] == array[n,i])         // если мин элемент равен текущему - оставляем
    continue;

    int temp = array[n,i];                  // замена мин элмента на первый
    array[n,i] = array[n,min];              // в не отсортированной части
    array[n,min] = temp;
    }
}
